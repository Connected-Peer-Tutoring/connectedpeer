...
class TutorSearch extends Component {
    ...
    onChangeSubjects(e, v) {
        if (v.length>5) this.setState({subjects: v, openErrTime:false, openErrTime1:false, openErrSubjects: true});
        else this.setState({subjects: v, openErrTime:false, openErrTime1:false, openErrSubjects: false});
    }
    ...
    async onSubmit(e) {
        ...
        aEnd = aStart.clone().startOf('day').hour(aEnd.hour()).minute(aEnd.minute()).utc().valueOf();
        ...
        if (aStart + 1800000 > aEnd) {
            ...
            var time = [];
            for(var i = Math.ceil(aStart/1800000); i < Math.floor(aEnd/1800000); i++){
                time.push(i*1800000);
            }
            this.setState({
                tutors: [],
        ...
        return (
            <div className="row">
                <div className="col s12 xl6 offset-xl3">
                    <div className="row">
                        <form className="col s12" onSubmit={this.onSubmit} id="Find_Tutor_Form">
                            <div className="row">
                            ...
                            <div className="row">
                                <div className="input-field col s12">
                                    <Autocomplete
                                        ...
                                        id="subjects"
                                        size="small"
                                        options={SubjectList.filter((v)=>this.props.user_data.subjects.indexOf(v)<0)}
                            ...
                            <input type="submit" value="Find tutors" className="waves-effect waves-light btn cyan"/>
                        </form>
                    </div>
                    <TutorSearchHelper query={this.state.query} user_data={this.props.user_data} updateState={this.props.updateState}/>
                </div>
                <Snackbar open={this.state.openErrTime} autoHideDuration={6000} onClose={this.handleClose}>
                ...
                <Snackbar open={this.state.openErrTime1} autoHideDuration={6000} onClose={this.handleClose}>
                    <MuiAlert elevation={6} variant="filled" onClose={this.handleClose} severity="error">
                        Your search time must be atleast 24 hours ahead
                    </MuiAlert>
                </Snackbar>
                <Snackbar open={this.state.openErrSubjects} autoHideDuration={6000} onClose={this.handleClose}>
                    <MuiAlert elevation={6} variant="filled" onClose={this.handleClose} severity="error">
