...
async function post(req, res) {
  try{
    req.user.subjects = req.body.subjects.sort((a, b) => SubjectList.indexOf(a) - SubjectList.indexOf(b));
    req.user.bio = req.body.bio;
    req.user.tutorAvailability = req.body.tutorAvailability.sort(),
    req.user.save();

    for (var i = 0; i < req.body.tutorAvailability.length; i++) {
      let tutorTime = await TutorTimes.findOne({ time: req.body.tutorAvailability[i] });
...
async function getTutors(req, res) {
  try{
    ...
    while(tutorIDs.length < 16){
      if (i === req.body.q.time.length){ return res.json({tutorIDs: tutorIDs, hasMore: false}) };
      ...
      if (tutorTime) {
        if (req.body.page*16 >= t + tutorTime.tutors.length){
          ...
          for(var j = req.body.page*16-t; j < tutorTime.tutors.length; j++){
            User.findById(tutorTime.tutors[j], (err, tutor) => {
              if (err || tutor === null) {
                ...
                if (!tutor.tutorAvailability.indexOf(req.body.q.time[i])) {
                  ...
                  if (subjectsFilterMet && tutor.appointments.flat().indexOf(req.body.q.time[i]-1800000) && tutor.appointments.flat().indexOf(req.body.q.time[i]) && tutor.appointments.flat().indexOf(req.body.q.time[i]+1800000)){
