            ...
            <span onClick={(e) => this.setState({openErr: false, openSuccess: false})}>
                <div id={"cancelAppointmentModal"} className="modal">
                    <div className="modal-content">
                        <h3 className="red-text">Cancel Appointment with {this.props.user_data.firstName}</h3>
                        <h4>{moment(this.props.appointment[0]).local().format("h:mm A, dddd MMMM Do, YYYY")}</h4>
                        <h5 className="red-text">You may be penalized for cancelling an appointment</h5>
                    </div>
                    <div className="modal-footer">
                        <a href="#!" className="waves-effect waves-light btn red modal-close" onClick={this.onSubmit}>Cancel Appointment
