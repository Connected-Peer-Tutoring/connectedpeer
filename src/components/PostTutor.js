...
class PostTutor extends Component {
    ...
    async onSubmit(e) {
        ...
        if (aStart + 3600000 > aEnd) {
        ...
        } else if (this.state.subjects.length === 0 && this.props.user_data.subjects.length === 0) {
            this.setState({
                openErrSubject: true
            });
        } else if (this.state.bio.length === 0 && this.props.user_data.bio.length === 0) {
            this.setState({
                openErrBio: true
            });
        } else {
            ...
            for(var i = Math.ceil(aStart/1800000); i < Math.floor(aEnd/1800000); i++){
                if (tutorAvailability.indexOf(i*1800000) < 0) tutorAvailability.push(i*1800000);
            }
            const newTutorPost = {
                subjects: (this.state.subjects.length===0)?this.props.user_data.subjects:this.state.subjects,
        ...
        return (
            <div className="row">
                <div className="col s12 xl6 offset-xl3">
                    <div className="row">
                        <form className="col s12" onSubmit={this.onSubmit} id="Post_Tutor_Form">
                            <div className="row">
                            ...
                            <input type="submit" value="Post" className="waves-effect waves-light btn teal"/>
                        </form>
                    </div>
                </div>
                ...
                <Snackbar open={this.state.openErrSubject} autoHideDuration={6000} onClose={this.handleClose}>
                    <MuiAlert elevation={6} variant="filled" onClose={this.handleClose} severity="error">
                        Please input the subject(s) you can tutor
                    </MuiAlert>
                </Snackbar>
                <Snackbar open={this.state.openErrBio} autoHideDuration={6000} onClose={this.handleClose}>
                    <MuiAlert elevation={6} variant="filled" onClose={this.handleClose} severity="error">
